#!/usr/bin/python
"""
A simple and buggy script for plotting columns of data from files with matplotlib_ backend.

`Imports`: ``matplotlib``, ``sys``, ``shlex``, ``os``

Examples
--------
- plot all PDG data and fit from Pennington:

	>>> python plot.py data/data data/pennington
...
class Plot:
	...
	def plot(self, data):
		...
		if self.kwargs['linestyle'][0] == '[':
			self.p[0].set_dashes(map(int, self.kwargs['linestyle'][1:-1].split(',')))
	...
	with open(filename) as inputfile:
		for eachline in inputfile:
			...
			if a == '' or a[0] == '#':
				...
				plt.gcf().set_size_inches(*map(float, val.split()))
			elif a == 'minx':
				...
				axis = plt.gca()
				vals = shlex.split(val)
				axis.xaxis.set_ticks(map(float, vals))
				axis.xaxis.set_ticklabels(vals)
			elif a == 'yticks':
				axis = plt.gca()
				vals = shlex.split(val)
				axis.yaxis.set_ticks(map(float, vals))
				axis.yaxis.set_ticklabels(vals)
			elif a == 'color':
...
def initFig():
	...
	#rc('text', dvipnghack=False, usetex=True)
	rc('mathtext', fontset='custom', rm='serif')
...
def saveFig(filename, size=[10,6]):
	fig = plt.gcf()
...
if __name__ == '__main__':
	...
	if '-o' in args:
		...
		i = args.index('-os')
		output_filename = args[i+1]
		osize = map(float, args[i+2].split('x'))
		filenames = set(args[:i]+args[i+3:])
	else:
